...
class PolypSizeEstimationDataset(Dataset):
    ...
    def __init__(self, params, phase='training', test_id=0, num_folds=6, transform=None, mode='per_object'):
        ...
        assert lists.duplicated(subset=['unique_id', 'frame_id']).sum() == self.dataframe.duplicated(
            subset=['unique_id', 'frame_id']).sum()
        ...
        if params['filter_centered_samples_ratio'] < 1.0:
            self.dataframe = self._filter_centered_samples(center_ratio=params['filter_centered_samples_ratio'],
                                                           mode="center")
