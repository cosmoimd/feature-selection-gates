...
from modules.models.multi_stream_nets import FSGNetR18
...
def build_configuration(config, k_fold, num_folds=6):
    ...
    # Pass the dictionary keys and values as arguments to the function
    print("  Training Set...")
    dataset_train = dataset_class(params=default_dataset_params, phase='training', test_id=k_fold, num_folds=num_folds, transform=transforms)

    print("  Validation Set...")
    dataset_valid = dataset_class(params=default_dataset_params, phase='validation', test_id=k_fold, num_folds=num_folds, transform=resize_normalize_only)

    print("  Testing Set...")
    dataset_test = dataset_class(params=default_dataset_params, phase='testing', test_id=k_fold, num_folds=num_folds, transform=resize_normalize_only)
    ...
    if model_params['num_classes'] == 1:
        train_sampler = sampler.InstanceSampler(data_source=dataset_train,
                                                num_samples_per_instance=config['dataset_params']["n_frames_to_sample_train"],
                                                rand_seed=0, freeze=False)

        valid_sampler = sampler.InstanceSampler(data_source=dataset_valid,
                                                num_samples_per_instance=config['dataset_params']["n_frames_to_sample_valid"],
                                                rand_seed=0, freeze=False)
    else:
